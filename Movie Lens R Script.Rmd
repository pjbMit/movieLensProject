...
x2 <- as.matrix(getSubset(x[,o],myTrain$rating,0.05))
dim(x2)
...
#Alter myTrain to have fewer columns
myTrain <- myTrain[colNames]
...
# CART
set.seed(7)
fit.model1 <- train(rating ~ ., data =myTrain, method="rpart", metric = metric, maximize = FALSE,
    trControl=trainControl)

# Second Model
set.seed(7)
fit.model2 <- train(rating~., data=myTrain, method="treebag", metric = metric, maximize = FALSE, na.action = na.omit,
                trControl=trainControl)
...
# model1 <- randomForest(rating ~ ., data = myTrain[colNames], importance = TRUE, mtry=6)
# model1
# print(model1)

dart.parm <- list(booster = "dart", rate_drop = 0.1, nthread = 4, eta = 0.1, max_depth = 3, subsample = 1, eval_metric = "rmse")
rowNames <- rownames(myTrain)
dart <- xgboost(data = as.matrix(myTrain[,-1]), label = rownames(myTrain[,-1]), params = dart.parm, nrounds = 50, verbose = 0, seed = 2017)
colNames <- names(myTrain[, -1])
colNames
pred1 <- predict(dart, as.matrix(myTrain[, -1]))
pred2 <- predict(dart, as.matrix(myTest[colNames]))
roc(as.factor(myTrain$rating), pred1)
...
##ctree2 with CV
  ...
  fit.ctree2CV <- train(formula, data=myTrain, method = 'ctree2', trControl=fitControl,tuneGrid=Grid,metric='RMSE')
  print(fit.ctree2CV)
